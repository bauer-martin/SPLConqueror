    ...
    class Z3VariantGenerator : IVariantGenerator
    {
        ...
        public List<Configuration> GenerateAllVariants(List<ConfigurationOption> optionsToConsider)
        {
            ...
            while (solver.Check() == Status.SATISFIABLE)
            {
                ...
                bool configIsValid = _vm.configurationIsValid(c);
                bool isInConfigurationFile = !VariantGeneratorUtilities.IsInConfigurationFile(c, allConfigurations);
                bool fulfillsMixedConstraintrs = VariantGeneratorUtilities.FulfillsMixedConstraints(c, _vm);
                if (configIsValid && isInConfigurationFile && fulfillsMixedConstraintrs)
                {
                    allConfigurations.Add(c);
                }
                solver.Push();
                solver.Assert(Z3Solver.NegateExpr(z3Context, Z3Solver.ConvertConfiguration(z3Context, confOpts.Item1, optionToTerm, _vm, numericValues: confOpts.Item2)));
            }
        ...
        public List<List<BinaryOption>> GenerateUpToN(int n)
        {
            ...
            Tuple<Context, BoolExpr> z3Tuple = Z3Solver.GetInitializedBooleanSolverSystem(out variables, out optionToTerm, out termToOption, _vm, this.henard, random.Next());
            Context z3Context = z3Tuple.Item1;
            BoolExpr z3Constraints = z3Tuple.Item2;
            ...
            while (s.Check() == Status.SATISFIABLE && (configurations.Count < n || n < 0))
            {
                ...
                if (henard)
                {
                    BoolExpr newConstraint = Z3Solver.NegateExpr(z3Context, Z3Solver.ConvertConfiguration(z3Context, config, optionToTerm, _vm));
                    ...
                    z3Tuple = Z3Solver.GetInitializedBooleanSolverSystem(out variables, out optionToTerm, out termToOption, _vm, this.henard, random.Next());
                ...
                {
                    s.Add(Z3Solver.NegateExpr(z3Context, Z3Solver.ConvertConfiguration(z3Context, config, optionToTerm, _vm)));
                }
            }
        ...
        public List<BinaryOption> GenerateConfigWithoutOption(BinaryOption optionToBeRemoved, List<BinaryOption> originalConfig, out List<BinaryOption> removedElements)
        {
            ...
            Dictionary<BoolExpr, BinaryOption> termToOption;
            Dictionary<BinaryOption, BoolExpr> optionToTerm;
            Tuple<Context, BoolExpr> z3Tuple = Z3Solver.GetInitializedBooleanSolverSystem(out variables, out optionToTerm, out termToOption, _vm, this.henard);
            Context z3Context = z3Tuple.Item1;
            BoolExpr z3Constraints = z3Tuple.Item2;
        ...
        public List<List<BinaryOption>> FindAllMaximizedConfigs(List<BinaryOption> config, List<BinaryOption> unwantedOptions)
        {
            ...
            while (optimizer.Check() == Status.SATISFIABLE && currentSize >= bestSize)
            {
                ...
                currentSize = solution.Count;
                optimizer.Assert(z3Context.MkNot(Z3Solver.ConvertConfiguration(z3Context, solution, optionToTerm, _vm)));
            }
        ...
        {
            return new Z3BucketSession(_vm, z3RandomSeed, henard);
        }
    }
}
